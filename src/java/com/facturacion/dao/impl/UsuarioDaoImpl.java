...
public class UsuarioDaoImpl implements UsuarioDao {
    ...
    public int save(FacUsuario usuario) throws SQLException {
        ...
        try {
            ...
            sql.append("INSERT INTO FAC_USUARIO(USU_CODIGO, USU_LOGIN, USU_CLAVE, USU_NOMBRES, USU_APELLIDOS, USU_CORREO, USU_IDENTIFICACION, USU_DIRECCION, USU_TELEFONO, USU_ESTADO, USU_FECHA_CREACION) VALUES (FAC_SEQ_USUARIO.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public int update(FacUsuario usuario) throws SQLException {
        ...
        try {
            conn = new ConexionDB().getConexion();
            pstmt = conn.prepareStatement("UPDATE FAC_USUARIO SET USU_LOGIN='" + usuario.getUsuLogin() + "',USU_CLAVE='" + usuario.getUsuClave() + "',"
    ...
    public int delete(int id) throws SQLException {
        ...
        try {
            conn = new ConexionDB().getConexion();
            sql.append("DELETE FROM FAC_USUARIO WHERE USU_CODIGO = ?");
    ...
    public List<FacUsuario> findAll() throws SQLException {
        ...
        try {
            conn = new ConexionDB().getConexion();
            pstmt = conn.prepareStatement("SELECT * FROM FAC_USUARIO ");
    ...
    public boolean existePorCampo(String username) throws SQLException {
        try {
            Integer rowCount = 0;
            conn = new ConexionDB().getConexion();
            pstmt = conn.prepareStatement("SELECT COUNT(*) AS CONTADOR FROM FAC_USUARIO U WHERE U.USU_LOGIN='" + username + "'");
            rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    public Long nuevoCodigo() throws SQLException {
        try {
            ...
            conn = new ConexionDB().getConexion();
            pstmt = conn.prepareStatement("SELECT MAX(U.USU_CODIGO)+1 FROM FAC_USUARIO U");
            rs = pstmt.executeQuery();
            if (rs.next()) {
                max = rs.getLong(1);
            }
